...
class TowerDetectionPhaseNode(Node):
    ...
    def robot_feedback_callback(self, msg):
        ...
        if at_tower:
            ...
            self.phase = 'wait_image_for_tower'
            self.capture_counter = 0
            self.get_logger().info(f'Robot confirma pose {self.tower_pose_name}. Esperando imagen estable.')
    ...
    def image_callback(self, msg):
        ...
        if self.phase != 'wait_image_for_tower' or not self.robot_at_tower_pose:
            if self.show_debug:
                cv2.imshow('Tower input', frame)
                cv2.waitKey(1)
            return

        result = detect_tower_pieces(frame)
        debug_vis = draw_tower_results(frame, result)

        if self.show_debug:
            cv2.imshow('Tower input', frame)
            cv2.imshow('Tower seed', result['seed_mask'])
            cv2.imshow('Tower foreground', result['foreground_mask'])
            cv2.imshow('Tower detections', debug_vis)
            cv2.waitKey(1)

        if len(result['pieces']) == 0:
            self.capture_counter = 0
            self.get_logger().warn('No se detectaron piezas de torre en esta imagen.')
            return

        self.capture_counter += 1
        self.get_logger().info(
            f'Detección de torre válida {self.capture_counter}/{self.stable_frames_required} con {len(result["pieces"])} piezas.'
        )

        if self.capture_counter < self.stable_frames_required:
            return
        ...
        self.robot_at_tower_pose = False
        self.capture_counter = 0
        self.get_logger().info(f'Fase torre terminada. Se solicitó al robot la pose {self.next_pose_name}.')
