...
class CalibrationNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("calibration node fully setup new node")
    ...
    def load_calibration_image(self):
        ...
        file_path = os.path.join(pkg_share, "config", "sample_better.jpg")
    ...
    def take_picture(self, request, response):
        ...
        self.get_logger().info(f"{file_path}")
        if response.success == True:
            response.message = f"Image properly taken and saved at {file_path}"
        else:
