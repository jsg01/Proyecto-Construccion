...
int main(int argc, char ** argv)
{
    ...
    // Crear pared vertical delante
    moveit_msgs::msg::CollisionObject wall_object;
    wall_object.header.frame_id = "base_link";
    wall_object.id = "pared_delante";
    ...
    std::string modo = node->declare_parameter("modo", "down");  // 🔥 NUEVO

    RCLCPP_INFO(node->get_logger(),
        "Objetivo: x=%.3f y=%.3f z=%.3f modo=%s",
        x, y, z, modo.c_str());
    ...
    else
    {
        RCLCPP_WARN(node->get_logger(),
            "Modo desconocido (%s). Usando 'down'", modo.c_str());
    ...
    if (dist < 0.3 || dz < 0.02)
    {
        ...
        {
            RCLCPP_WARN(node->get_logger(),
                "Cartesiano incompleto (fraction=%.2f). Fallback a JOINT...", fraction);
    ...
    {
        RCLCPP_INFO(node->get_logger(), "Movimiento JOINT");
