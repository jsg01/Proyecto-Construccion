...
from construccion_interfaces.srv import PixelToRobot, RobotToPixel, MoveNamedPose
...
class LoosePieceDetectionNode(Node):
    def __init__(self):
        ...
        move_named_pose_service = str(self.get_parameter('move_named_pose_service').value)
        ...
        self.move_named_pose_client = self.create_client(
            MoveNamedPose,
            move_named_pose_service
        )
        ...
        self.get_logger().info(f'Esperando servicio {move_named_pose_service}...')
        while rclpy.ok() and not self.move_named_pose_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(f'Servicio no disponible todavía: {move_named_pose_service}')

        while rclpy.ok() and not self.pixel_to_robot_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Esperando servicio /pixel_to_robot_plane...')

        while rclpy.ok() and not self.robot_to_pixel_client.wait_for_service(timeout_sec=1.0):
        ...
        if self.move_named_pose(self.detect_pose_name):
            self.robot_at_detect_pose = True
            self.get_logger().info(f'Robot confirmado en pose {self.detect_pose_name}.')
        else:
            self.robot_at_detect_pose = True
            self.get_logger().warn(
                f'move_named_pose({self.detect_pose_name}) falló o timeout, '
                'pero continúo porque se asume que el robot ya está en esa pose.'
            )
        ...
        self.get_logger().info(f'Pose inicial solicitada por servicio: {self.detect_pose_name}')
        self.get_logger().info('Flujo correcto:')
        self.get_logger().info(f'1) El nodo llama al servicio /move_named_pose para ir a {self.detect_pose_name}')
        self.get_logger().info('2) Cuando el servicio devuelve success=True, deja la mesa vacía y pulsa b')
    ...
    def move_named_pose(self, pose_name: str) -> bool:
        ...
        deadline = self.get_clock().now().nanoseconds / 1e9 + self.service_timeout_sec

        while rclpy.ok() and not future.done():
            rclpy.spin_once(self, timeout_sec=0.1)
            now_sec = self.get_clock().now().nanoseconds / 1e9
            if now_sec > deadline:
                self.get_logger().error(f'Timeout esperando move_named_pose({pose_name})')
                return False

        res = future.result()
        if res is None:
            self.get_logger().error(f'move_named_pose({pose_name}) devolvió resultado vacío')
            return False

        if not res.success:
            self.get_logger().error(f'move_named_pose({pose_name}) falló: {res.message}')
            return False
    ...
    def capture_background(self, frame_bgr):
        ...
        self.get_logger().info(
            f'Fondo vacío capturado correctamente en la pose {self.detect_pose_name}.'
        )
    ...
    def image_callback(self, msg):
        ...
        if self.show_debug:
            ...
            if not self.robot_at_detect_pose:
                info_lines.append(f'Esperando servicio correcto para {self.detect_pose_name}')
