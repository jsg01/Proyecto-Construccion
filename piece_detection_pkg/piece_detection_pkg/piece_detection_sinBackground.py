...
class PieceDetectionNode(Node):
    def __init__(self):
        ...
        bg_img = cv2.imread("/home/ricardo/Desktop/UPM/LABrobotica/Proyecto-Construccion/LabRob/src/calibration_package/config/background_table.jpg")  # <-- CHANGE PATH

        if bg_img is None:
            self.get_logger().error("No se pudo cargar la imagen de fondo")
        else:
            self.background_rgi_raw = bg_img
        self.frame_count = 0
    ...
    def image_callback(self, msg):
        try:
            ...
            return
        # Prepare background once (resize + RGI)
        if self.background_rgi is None and hasattr(self, "background_rgi_raw"):
            bg_resized = cv2.resize(
                self.background_rgi_raw,
                (frame.shape[1], frame.shape[0])
            )
            self.background_rgi = self.rgb_to_rgi(bg_resized)
            self.background_ready = True
            self.get_logger().info("Fondo cargado desde PNG")
