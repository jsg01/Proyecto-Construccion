...
class PieceDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('background_path', '')
        ...
        self.background_path = self.get_parameter('background_path').value
    ...
    def load_background(self):
        if not self.background_path:
            self.get_logger().warn('No se ha definido background_path')
            return

        bg_bgr = cv2.imread(self.background_path)
        if bg_bgr is None:
            self.get_logger().error(f'No se pudo leer el fondo: {self.background_path}')
            return

        self.background_rgi = self.rgb_to_rgi(bg_bgr)
        self.get_logger().info(f'Fondo cargado desde: {self.background_path}')
    ...
    def subtract_background(self, frame_bgr):
        ...
        if self.background_rgi is None:
            self.get_logger().warn('No hay fondo cargado')
